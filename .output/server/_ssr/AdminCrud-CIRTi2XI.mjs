...
import { It as ArrowDown, Mt as ArrowUp, N as Pencil, O as Search, U as LoaderCircle, a as X, j as Plus, k as Save, p as Trash2 } from "../_libs/lucide-react.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/AdminCrud-CIRTi2XI.js
...
function AdminCrud({ table, title, description, fields, displayColumns, orderBy = {
	...
	async function load() {
		...
		try {
			if (useBackend) {
				...
				const { data, error } = await supabase.from(table).select("*").order(orderBy.column, { ascending: orderBy.ascending });
	...
	async function remove(id) {
		...
		try {
			if (useBackend) await apiFetch(`/${id}`, "DELETE");
			else {
	...
	async function reorder(row, dir) {
		...
		try {
			if (useBackend) await apiFetch(`/${row.id}`, "PUT", { sortOrder: next });
			else {
				const { error } = await supabase.from(table).update({ sort_order: next }).eq("id", row.id);
				if (error) return toast.error(error.message);
			}
		} catch (e) {
			return toast.error(e.message ?? String(e));
