...
function SolarEnquiryFormPage() {
    ...
    return (
      <div className="min-h-screen bg-slate-900 text-white font-sans flex flex-col justify-between">
        ...
        <main className="py-20 max-w-xl mx-auto px-4 text-center space-y-6">
          ...
          <h1 className="text-3xl font-black text-white">Enquiry Received!</h1>
          ...
          <div className="pt-6">
            <Link to="/products" className="px-6 py-3 bg-primary text-slate-900 font-bold rounded-xl transition shadow-lg">
              Return to Catalog
            </Link>
          </div>
        </main>
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans">
      <Header />
      <PageHero
        title="Custom Rooftop Quote"
        subtitle="Submit your site requirements, average bill rates, and roof characteristics to receive an itemized commercial proposal."
      />
      ...
      <main className="py-12 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-slate-800/20 border border-slate-700/40 p-6 sm:p-8 rounded-3xl space-y-6">
          ...
          {presetPackageName && (
            <div className="bg-primary/10 border border-primary/30 p-4 rounded-2xl flex items-center gap-3 text-xs text-primary font-bold">
          ...
          <form onSubmit={handleEnquirySubmit} className="space-y-6">
            ...
            <div className="bg-slate-900 border border-slate-750 p-6 rounded-2xl space-y-4">
              <label className="text-xs font-bold text-slate-400 block uppercase">Upload Latest Electricity Bill / Rooftop Layout (Optional)</label>
              
              <div className="flex flex-col items-center justify-center border-2 border-dashed border-slate-700 rounded-xl p-6 bg-slate-950 relative hover:border-primary/45 transition">
                <UploadCloud className="h-10 w-10 text-slate-400 mb-2" />
                <span className="text-xs font-bold text-slate-350">{uploading ? "Uploading..." : "Click to select a file"}</span>
                <span className="text-[10px] text-slate-500 mt-1">PDF, JPG, PNG up to 5MB</span>
                <input
                  type="file"
              ...
              {uploadedUrl && (
                <p className="text-xs text-green-400 font-bold flex items-center gap-1.5 mt-2">
            ...
            <div className="space-y-1">
              ...
              <textarea
                ...
                className="w-full bg-slate-900 border border-slate-700 rounded-xl p-2.5 text-xs text-white h-24 resize-none focus:outline-none focus:border-primary"
              />
            </div>
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 bg-primary text-slate-900 font-black rounded-2xl hover:bg-primary-hover transition shadow-lg disabled:opacity-50"
            >
